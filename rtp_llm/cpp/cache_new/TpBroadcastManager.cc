...
void TpBroadcastManager::registerHandler(const std::string& handler_name, TpBroadcastHandlerPtr handler) {
    handlers_[handler_name] = handler;
}

bool TpBroadcastManager::executeHandler(const std::string&             handler_name,
...
bool TpBroadcastManager::broadcast(const std::vector<BroadcastAllTpRequestPB>& requests,
